...
نکته: برای فهرست، `ai_result` برگردانده نمی‌شود. در صورت نیاز به پیش‌نمایش خلاصه، می‌توانید در فرانت‌اند پس از کلیک روی آیتم، جزئیات را فراخوانی و `summary` را از `ai_result` استخراج کنید. (در صورت نیاز می‌توانیم فیلد `ai_summary` را به خروجی لیست اضافه کنیم.)
